...
import os
import unittest
from auction import auction_lib, organic, assign, params
...
class TestAuction(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.A = pd.read_csv(TEST_DIR + "/A.csv")
        cls.A1000 = pd.read_csv(TEST_DIR + "/A_1000.csv")
        cls.Page = pd.read_csv(TEST_DIR + "/Page.csv")
    ...
    def test_bid_tensor(self):
        # 500us for 1000x20
        BQ = auction_lib.bid_tensor(self.A, self.Page, self.organic_idx, self.PR)
    ...
    def test_pre_allocate(self):
        ...
        BQ, owner_priority, organic_idx, idx_map, owner_ids, reserves = auction_lib.pre_allocate(self.A, self.Page, self.PR)
    ...
    def test_pre_allocate_raise_reserve(self):
        self.PR.reserve_price = 1. / 1000
        BQ, owner_priority, organic_idx, idx_map, owner_ids, reserves1 = auction_lib.pre_allocate(self.A, self.Page, self.PR)
        ...
        BQ, owner_priority, organic_idx, idx_map, owner_ids, reserves2 = auction_lib.pre_allocate(self.A, self.Page, self.PR)
    ...
    def test_pre_allocate_raise_exchange(self):
        ...
        BQ1, owner_priority1, _, _, _, _ = auction_lib.pre_allocate(self.A, self.Page, self.PR)
        self.PR.exchange_rate = 100000
        BQ2, owner_priority2,  _, _, _, _ = auction_lib.pre_allocate(self.A, self.Page, self.PR)
    ...
    def test_pre_allocate_top_k(self):
        ...
        BQ, owner_priority, organic_idx, idx_map, owner_ids, _ = auction_lib.pre_allocate(self.A, self.Page, self.PR)
        max_bids = np.max(BQ[0], axis=1)
    ...
    def test_pre_allocate_top_k_big(self):
        A = self.A1000
        self.PR.pre_allocate_top_k = 10
        BQ, owner_priority, organic_idx, idx_map, owner_ids, _ = auction_lib.pre_allocate(A, self.Page, self.PR)
    ...
    def test_alloc_BQ(self):
        BQ, owner_priority, organic_idx, idx_map, owner_ids, _ = auction_lib.pre_allocate(
    ...
    def test_run_to_solve(self):
        BQ, owner_priority, organic_idx, reverse_row_map, owner_ids, _ = auction_lib.pre_allocate(
            self.A, self.Page, self.PR)
        alloc_idx = organic.shift_organic_for_alloc(organic_idx, self.alloc)
        alloc_idx_page = alloc_idx[:len(self.Page)]
        BQ_alloc = auction_lib.alloc_BQ(
            BQ=BQ,
            alloc_idx_page=alloc_idx_page,
            owner_ids=owner_ids,
            owner_priority=owner_priority,
    ...
    def test_run(self):
        auctionResponse = auction_lib.run(self.A, self.Page, alloc=self.alloc, Param=self.PR)
    ...
    def test_ranks(self):
        r = auction_lib.ranks([0.2, 10, 3.3])
        self.assertListEqual(list(r), [0, 2, 1])

        r = auction_lib.ranks([3.3, 10, 3.5, 100, -1])
        self.assertListEqual(list(r), [1, 3, 2, 4, 0])

    def test_ranks_zeros(self):
        r = auction_lib.ranks([0, 10, 0, 0, 1.2])
    ...
    def test_sum_alloc_util(self):
        ...
        u = auction_lib.sum_alloc_util(A, [0, 4, 9], [1, 0.5, 0.25], {0,4,9})
        self.assertEqual(u, 21*0.5 + 47*0.25)

    def test_sum_alloc_util_pivot(self):
        A = np.arange(100).reshape((20, 5))
        u = auction_lib.sum_alloc_util(A, [0, 4, 9], [1, 0.5, 0.25], {})
        self.assertEqual(u, 0)

        A = np.arange(100).reshape((20, 5))
        u = auction_lib.sum_alloc_util(A, [0, 4, 9], [1, 0.5, 0.25], {0})
        self.assertEqual(u, 0)

        A = np.arange(100).reshape((20, 5))
        u = auction_lib.sum_alloc_util(A, [0, 4, 9], [1, 0.5, 0.25], {0, 4})
        self.assertEqual(u, 21*0.5)

    def test_move_imp_curve(self):
        curve = [1, 0.75, 0.5, 0.25, 0.1]
        curve_v = np.array(curve)
        self.assertListEqual(list(auction_lib.move_imp_curve(curve_v, 0)), curve)
        self.assertListEqual(
            list(auction_lib.move_imp_curve(curve_v, 1)),
    ...
    def test_util(self):
        ...
        r = np.zeros(20)
        U = auction_lib.util(B, Q, 1, r)
        self.assertTrue(np.all(U == B*0.5))
        U = auction_lib.util(B, Q, 2, r)
